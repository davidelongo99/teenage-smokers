...
```{r}
# Since for the interactive network we need a dataframe, we will 
# create a dataframe to store the indices of the 1 values (that corresponds to smokers)
df1_tb = data.frame(row=character(), column=character())

# loop through the matrix and get the indices of the 1 values
for (i in 1:nrow(f_tb1)) {
  for (j in 1:ncol(f_tb1)) {
    if (f_tb1[i, j] == 1) {
      df1_tb = rbind(df1_tb, data.frame(row= rownames(f_tb1)[i], column=colnames(f_tb1)[j]))
    }
  }
}
...
# Interactive network
# Plot
p <- simpleNetwork(df1_tb, height="100px", width="100px",
                  nodeColour = "gray48",    
                  linkColour = "#666",
                  fontFamily = "serif",
                  opacity = 0.9,           
...
```{r}
### Interactive visualization - **IT DOES NOT WOrk**

# Convert the Links data frame
#links_df <- as_data_frame(g_tb1_gdr)



# create the Nodes Dataframe
#nodes_df <- data.frame(name = c(links_df[,"from"], links_df[,"to"]))
#remove duplicates 
#nodes_df <- unique(nodes_df)
#nodes_df <- merge(nodes_df, df_gender, by ="name", all.x = TRUE)

```

```{r}
#forceNetwork(Links =links_df, Nodes = nodes_df , Source = "from", Target = "to",
#             NodeID = "name", Group = "sex",
...
2. Take the first column (smoking behavior at time 3) as a row vector. 
```{r}
...
4. Multiply *f3* matrix by *tb3* vector.
```{r}
f_tb3 = t(t(f3) * tb3)
